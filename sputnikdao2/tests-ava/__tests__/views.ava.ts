...
test('View method get_policy', async (t) => {
    ...
    t.deepEqual(
        normalizePolicy(await bob.view('get_policy')),
        normalizePolicy(policy),
    );
});

test('View method get_staking_contract', async (t) => {
    const { alice, root, dao } = t.context.accounts;
...
test('View get_locked_storage_amount', async (t) => {
    const { alice, root, dao } = t.context.accounts;
...
test('View get_available_amount', async (t) => {
    const { alice, root, dao } = t.context.accounts;
    ...
    t.log('Available amount after proposal: ' + afterProposal);
    t.assert(beforeProposal.gt(afterProposal));
});

test('View methods for delegation', async (t) => {
    const { alice, root, dao } = t.context.accounts;
    const testToken = await initTestToken(root);
    const staking = await initStaking(root, dao, testToken);
...
test('View methods for proposals', async (t) => {
    const { alice, root, dao } = t.context.accounts;
    ...
    t.deepEqual(proposalAlice.vote_counts, realProposalAlice.vote_counts);
...
test('View methods for bounties', async (t) => {
    const { alice, root, dao } = t.context.accounts;
    ...
    //Test get_bounties
    t.deepEqual(await dao.view('get_bounties', { from_index: 0, limit: 100 }), [
        bounty,
    ]);
    //Test get_bounty
