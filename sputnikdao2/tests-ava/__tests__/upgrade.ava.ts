...
test('Store DAO upgrade code in DAO via factory', async (t) => {
    const { root, factory } = t.context.accounts;
    const config = {
        name: 'upgradedao',
        purpose: 'to test',
    ...
    await root.call('upgradedao.factory.test.near', 'add_proposal', proposal, {
    ...
    t.log(new_proposal);
    t.is(new_proposal.description, 'Store DAO upgrade contract code blob');
    t.is(new_proposal.proposer, 'test.near');
    t.is(new_proposal.status, 'InProgress');
    t.truthy(new_proposal.kind.FunctionCall);
    t.is(new_proposal.kind.FunctionCall.receiver_id, `${factory.accountId}`);
    ...
    t.is(new_proposal_2.kind.FunctionCall.receiver_id, `${factory.accountId}`);
...
test('Upgrade self using factory', async (t) => {
    const { root, factory } = t.context.accounts;
    const config = {
        name: 'testdao',
        purpose: 'to test',
