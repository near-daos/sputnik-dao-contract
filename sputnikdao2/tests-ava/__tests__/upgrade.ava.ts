...
import { workspace, workspaceWithFactory, initTestToken, initStaking, setStakingId, workspaceWithoutInit, voteApprove } from './utils';
...
workspaceWithFactory.test('basic', async (test, { alice, root, factory, dao }) => {
...
workspaceWithFactory.test('Store DAO upgrade code in DAO via factory', async (test, { alice, root, factory, dao }) => {
		...
		await root.call(dao, 'add_proposal', proposal, { attachedDeposit: toYocto('1'), gas: tGas(300) })
    ...
    await root.call(dao, 'act_proposal', { id: 0, action: 'VoteApprove' })
		...
		await root.call(dao, 'add_proposal', proposalUpgradeSelf, { attachedDeposit: toYocto('1'), gas: tGas(300) })
		...
		test.is(new_proposal_1.description, 'Upgrade DAO contract using local code blob');
    ...
    await root.call(dao, 'act_proposal', { id: 1, action: 'VoteApprove' })
		...
		test.is(new_proposal_2.description, 'Remove DAO upgrade contract local code blob via factory');
